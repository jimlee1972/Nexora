...
| M7 | Device-neutral input routing, stable touch IDs and virtual-list materialization |
| M8 | Separate character intent and resolved motion; navigation output is data, not a transform write |
| M9 | Reference-counted presentation-resource residency |
...
The contract test `runtime.v1_m4_m12_contracts` exercises every row. Platform SDK adapters and
production authoring tools remain future work and must preserve these interfaces rather than
bypassing their lifecycle checks.
