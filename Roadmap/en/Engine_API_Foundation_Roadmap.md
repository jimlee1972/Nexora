...
- **API-M1 Math**: `Engine/Foundation/include/Nexora/Math/Math.h` now has dot/cross/normalize-safe/lerp/slerp, TRS compose/decompose, `Matrix3`/`Matrix4` inverse (epsilon-fallback policy), `LookAt`, `Orthographic`/`PerspectiveRadians`, `ExtractFrustum`, and `Intersects(Frustum, Aabb/Sphere)`. `Tests/API/ApiFoundationTests.cpp` provides the `sizeof`/`alignof`/`offsetof` ABI layout gate, a byte-level round trip, and a Compose/Decompose inverse check. `Vector4::Dot` (and the `Length`/`NormalizeSafe`/`Lerp` built on it) now has an SSE2 SIMD path (enabled via `NEXORA_MATH_HAS_SSE2` when the target is x86/x64 with SSE2, falling back to scalar otherwise), cross-checked against the scalar reference by a 10,000-iteration fuzz test whose tolerance is scaled to the sum of `|component product|` magnitudes rather than the final dot value, so catastrophic cancellation near zero doesn't produce a falsely tight bound. **Not done**: the SIMD path covers only `Vector4::Dot` -- `Vector3`'s dot/cross, `Matrix3`/`Matrix4` multiplication, and every other operation in this file remain scalar-only, and the SSE2 path is verified on x86/x64 only; on ARM/NEON it correctly falls back to scalar via the same guard but that fallback itself is untested (no ARM target in this sandbox). Coordinate golden tests against an external reference engine (only internal self-consistency is checked) are also still not done.
