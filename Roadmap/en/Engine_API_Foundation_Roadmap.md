...
- **API-M1 Math**: `Engine/Foundation/include/Nexora/Math/Math.h` now has dot/cross/normalize-safe/lerp/slerp, TRS compose/decompose, `Matrix3`/`Matrix4` inverse (epsilon-fallback policy), `LookAt`, `Orthographic`/`PerspectiveRadians`, `ExtractFrustum`, and `Intersects(Frustum, Aabb/Sphere)`. `Tests/API/ApiFoundationTests.cpp` provides the `sizeof`/`alignof`/`offsetof` ABI layout gate, a byte-level round trip, and a Compose/Decompose inverse check. **Not done**: a SIMD path (scalar only today, so there is no SIMD-vs-scalar tolerance comparison) and coordinate golden tests against an external reference engine (only internal self-consistency is checked).
...
- **API-M3 VFS**: `Engine/Core/include/Nexora/Core/Vfs.h`'s `VirtualFileSystem` now has two backends: `Mount` (directory) and the new `MountMemory` (in-memory, same Read/WriteAtomic/Metadata/Enumerate surface). `Tests/API/ApiCoreContractTests.cpp` runs the identical contract suite (read/write/metadata/enumerate/traversal rejection/error injection/a multi-MiB round trip) against both backends. Mount names remain caller-chosen; the `engine:// project:// bundle:// cache:// user:// temp://` canonical roots are not all wired up yet -- only `content`, mounted by `Engine::Initialize`, exists today. **Not done**: `BundleMount`/`PlatformPackageMount`, the full async IO scheduler (priority preemption, request merge/coalescing, aligned reads, streaming deadline hints), memory mapping, Shipping-mode call-site privilege gating (`Mount` treats every caller equally today; only in-mount path traversal is rejected), and a true multi-GB/offset boundary test (today's test is a multi-MiB smoke test, not a huge-file test).
