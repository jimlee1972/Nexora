...
Linux Vulkan now executes the complete frustum/distance/LOD/Hi-Z/compaction/classification/indirect-generation compute algorithm, verifies its output against the CPU reference, and crosses RenderGraph compute-to-graphics ownership before native indirect drawing. V2-M3 remains open because DX12/Metal indirect execution and full DX12/Vulkan/Metal target-tier parity have not passed their target-host gates.
