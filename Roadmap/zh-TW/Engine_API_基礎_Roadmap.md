...
- **API-M1 Math**：`Engine/Foundation/include/Nexora/Math/Math.h` 已有 dot/cross/normalize-safe/lerp/slerp、TRS compose/decompose、`Matrix3`/`Matrix4` inverse（epsilon-fallback policy）、`LookAt`、`Orthographic`/`PerspectiveRadians`、`ExtractFrustum` 與 `Intersects(Frustum, Aabb/Sphere)`。`Tests/API/ApiFoundationTests.cpp` 提供 `sizeof/alignof/offsetof` ABI layout gate、byte-level round trip、Compose/Decompose 反函數驗證。`Vector4`有自己專屬的 `Dot4`/`Length4`/`NormalizeSafe4`/`Lerp4`（刻意不跟 `Vector3` 的 `Dot`/`Length`/`NormalizeSafe`/`Lerp` 共用名稱：兩者都是 aggregate，裸的 brace list 如 `{1, 2, 3}` 對兩種型別都是同樣有效的轉換，一旦出現同名的 `Vector4` overload，就會讓所有既有的裸 brace 呼叫變成 ambiguous——這是先前用同名版本合併後被 review 抓到的真實 regression；用不同名字徹底避開這個問題，不必要求每個呼叫端都寫出明確型別）。`Dot4` 現在有 SSE2 SIMD 路徑（`NEXORA_MATH_HAS_SSE2` 偵測到 x86/x64 SSE2 時啟用，否則退回 scalar），並有 10,000 次迭代的 SIMD/scalar 容差比對 fuzz test（容差以各分量乘積絕對值總和為基準，避免 catastrophic cancellation 讓最終結果趨近零時容差反而失真）。**尚未做**：SIMD 只涵蓋 `Dot4` 這一條路徑 -- `Vector3` 的 dot/cross、`Matrix3`/`Matrix4` 乘法等其餘運算仍是純 scalar；SSE2 路徑本身也只在 x86/x64 驗證過，ARM/NEON 只是透過 `NEXORA_MATH_HAS_SSE2` guard 正確地退回 scalar，並未實際跑過驗證（此 sandbox 沒有 ARM 目標）。座標系 golden tests（只有內部一致性驗證，沒有跟外部參考引擎比對）仍未做。另外還有一個尚未修的既有問題：`NormalizeSafe({1, 2, 3})` 本身就已經在 `Vector3` 與 `Quaternion` 之間 ambiguous（`Quaternion::w` 預設值是 `1.0F`），這是同一種命名風險的既有案例，跟這次 SIMD 改動無關，需要另外為 `Quaternion` 決定命名或 API 形狀才能處理。
