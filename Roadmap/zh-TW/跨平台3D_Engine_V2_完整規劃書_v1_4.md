...
Linux Vulkan 現已有完整 frustum／distance／LOD／Hi-Z／compaction／classification／indirect-generation 實作，以及在 indirect draw 前必須對 CPU reference 比對全部輸出的 native gate。Indirect record 已固定為 C++／Slang 共用的 36-byte ABI，並以前四個 word 作為 Vulkan／D3D12／Metal 共通的 non-indexed draw prefix；backend 不得自行重訂。Phase 2 在 Slang-enabled native gate 實際通過前仍未驗收；DX12／Metal execution 與完整 target-tier parity 也仍待完成。
